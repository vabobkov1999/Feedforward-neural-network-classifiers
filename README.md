...
Нейронная сеть прямого распространения - это разновидность искусственной нейронной сеть, в которой соединения между узлами не образуют цикла. Мы подаём на вход "сырые" картинки, а на выходе получаем какую-то вероятность принадлежности какому-то классу.В FFNN данные сразу передаются в нейронную сеть.
...
В файле feed_forward_neural_network содержится программа программа для классификации изображений на основе нейронной сети.(Основные шаги программы расписаны по пунктам)
...
2. Цикл по изображениям из папки train - вытаскивает картинки и кладет в data, а их класс (имя симпсона) в labels.
...
flatten(его используют так как нейронная сеть работает только с одномерными массивами ) преобразуем картинку в одномерный массив, где его элементами будут пиксели RGB  расцветки
...
input_shape входная размерность указывается только в первом слое или методе и равна 32х32х3 = 3072.
...
6. В завершении построим графики потерь и точности
