...
Путь train_path к тренировочным папкам по которым будет учиться нейронка. test_path путь к файлам которые мы будем классифицировать. result_path куда сохраняются файлы которые нейронка обработала.
