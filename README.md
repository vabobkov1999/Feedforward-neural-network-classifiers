...
Все практически тоже самое пока не доходим до создания модели Sequential.
Conv2D это слой свёртки. Идея свёртки такая, что ты задаёшь какую-то матрицу с помощью которой ты получаешь набор других матриц.
kernel_size размер свёртки. Выходные фильтры будут иметь такую же размерность, как и входные фильтры

MaxPooling2D метод который обрезает матрицу в два раза по оси Х и Y. Он делит матрицу на квадраты 2х2 и в каждом таком квадрате сохраняет максимальное число

На выходе получается вектор длинны 1024 с помощью метода Flatten вытягиваем и получаем входные данные .

А дальше все как в предыдущем разделе   
